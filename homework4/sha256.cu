...
#ifdef __CUDACC__
#define SHA256_BOTH_INLINE __host__ __device__ __forceinline__
#else
#define SHA256_BOTH_INLINE inline
#endif

// circular shift - wiki:
//     https://en.wikipedia.org/wiki/Circular_shift
SHA256_BOTH_INLINE WORD _rotl(const WORD v, const WORD s)
{
	return (v << s) | (v >> (32 - s));
}

SHA256_BOTH_INLINE WORD _rotr(const WORD v, const WORD s)
{
	return (v >> s) | (v << (32 - s));
}

SHA256_BOTH_INLINE void _swap(BYTE &x, BYTE &y)
{
	const BYTE tmp = x;
	x = y;
	y = tmp;
...
#ifdef __CUDACC__
__device__ __constant__ WORD k_device[64] = { SHA256_K_VALUES };
#endif

static const WORD k_host[64] = { SHA256_K_VALUES };

#undef SHA256_K_VALUES
...
SHA256_HD void sha256_transform(SHA256 *ctx, const BYTE *msg)
{
	...
	WORD i, j;
#ifdef __CUDA_ARCH__
	const WORD *k_table = k_device;
#else
	const WORD *k_table = k_host;
#endif
...
#pragma unroll 64
	for(i=0;i<64;++i)
	{
		...
		WORD temp1 = h + S1 + ch + k_table[i] + w[i];
		WORD temp2 = S0 + maj;
